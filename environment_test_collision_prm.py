...
class PRM:
    ...
    def dijkstra_planning(self):
        start_node = PRMNode(self.start[0],self.start[1],0.0,-1)
        goal_node = PRMNode(self.goal[0],self.goal[1],0.0,-1)
        ...
        while True:
            ...
            for i in range(len(self.roadmap[c_id])):
                ...
                d = math.hypot(dx,dy)
                node = PRMNode(self.sample_x[n_id],self.sample_y[n_id],current.cost + d, c_id)
    ...
    def set_pixels(self, screen):
        ...
        for i, _ in enumerate(self.roadmap):
            for ii in range(len(self.roadmap[i])):
                ...
                line = pygame.draw.aaline(screen,blue,(self.sample_x[i],self.sample_y[i]),
        ...
        for pathx,pathy in zip(self.path_x,self.path_y):
